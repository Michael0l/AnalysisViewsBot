...
class UserBot():
    ...
    async def check_recent_posts(self, channel_id, good_views, limit):
        ...
        try:
            async for post in self.app.get_chat_history(channel_id, limit):
                # logging.warning(f'Канал {channel_id} - проверка {post.id} поста')
                ...
                if not await loader.db.is_tracked_post(channel_id, post.id):
                    ...
                    # logging.warning(f'Канал {channel_id} - пост {post.id}, есть в бд')
                    continue
            ...
            logging.warning(f'Канал {channel_id} - ошибка')
    ...
    async def check_post_cheating(self, channel_id, post_id):
        ...
        # logging.warning(f'Канал {channel_id} - проверка поста {post_id} - {post.views} просмотров, последний - {last_view}, допустимая раздница {view_check[0]} - { post.views - last_view}')
    ...
    async def join_channel(self, channel_id, link):
        try:
            await self.app.join_chat(link)
            user = await self.app.get_chat_member(channel_id, 'me')
            print(str(user.status))

            return str(user.status) in ['ChatMemberStatus.ADMINISTRATOR', 'ChatMemberStatus.MEMBER']
        except ChannelInvalid:
            return False